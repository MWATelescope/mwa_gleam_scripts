# !/bin/python
# Program for fitting and plotting SEDs for Randall et al (2011) calibrators sources.
...
hdulist = fits.open('crossmatching/atca_cali/tot_mwa_154-162MHzbase+atcacaldb+mrc+sumss+vlssr+nvss.fits')
...
S_1400 = np.array(tbdata['S1.4'])
S_1400_err = np.array(tbdata['S1.4']) * 0.1
S_843 = np.array(tbdata['St']) / 1000.
S_843_err = np.array(tbdata['St']) * 0.1 /1000.
S_408 = np.array(tbdata['S408'])
S_408_err = np.array(tbdata['e_S408'])
S_2700 = np.array(tbdata['S2.7'])
S_2700_err = np.array(tbdata['S2.7']) * 0.1
S_4800 = np.array(tbdata['S5'])
S_4800_err = np.array(tbdata['S5'])  * 0.1
S_20000 = np.array(tbdata['S20'])
S_20000_err = np.array(tbdata['S20']) * 0.1
# S_20000 = np.array(tbdata['S20'])
# S_20000_err = np.array(tbdata['e_S20'])
Name = np.array(tbdata['PKS'])
RA = np.array(tbdata['RA_J2000'])
Dec = np.array(tbdata['Dec_J2000'])
size = np.array(tbdata['Size'])

freq = np.array([76,84,92,99,107,115,123,130,143,150,158,166,174,181,189,197,204,212,219,227,408,843,2700,5000,20000])
flux = np.vstack((S_076,S_084,S_092,S_099,S_107,S_115,S_123,S_130,S_143,
    S_150,S_158,S_166,S_174,S_181,S_189,S_197,S_204,S_212,S_219,S_227,S_408,S_843,S_2700,S_4800,S_20000))
flux_err = np.vstack((S_076_err,S_084_err,S_092_err,S_099_err,S_107_err,S_115_err,S_123_err,S_130_err,S_143_err,
    S_150_err,S_158_err,S_166_err,S_174_err,S_181_err,S_189_err,S_197_err,S_204_err,S_212_err,S_219_err,S_227_err,S_408_err,S_843_err,S_2700_err,S_4800_err,S_20000_err))
...
def sed(models,paras,freq,flux,flux_err, title = "No title provided.", 
    ...
    if savefig == True:
        ...
        if not os.path.exists(os.getcwd()+'crossmatching/randall/figures'):
            os.makedirs(os.getcwd()+'crossmatching/randall/figures')
            print 'Creating directory ', os.getcwd()+'crossmatching/atca_cali/figures and saving figures in png format with title names.'
        plt.savefig('crossmatching/atca_cali/randall/'+title+'.png')
    plt.show()
...
for i in range(np.shape(flux[1])[0]): #[230] is pks0008-42
    ...
    if len(fluxplot) > 3.:
        ...
        if redchisq_curve[i] < 10. and redchisq_curve[i] > 0.:
            ...
            # if userinput == '' :
            sed([powlaw,singinhomobremss],[poptpowlaw,poptsinginhomobremss],freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = True, title = Name[i] )#+r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2))+r' Inhomobremss $ \chi_{\rm{red}}$  = '+str(round(redchisq_curve[i],2)))
        elif redchisq_pow[i] > 0.:
            # userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
            # if userinput == '' :
            sed(powlaw,poptpowlaw,freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = True, title = Name[i] )#+r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2)))
...
# tbhdu.writeto('gpssrc_pos.fits', clobber = True)
     ...
     #     try:
        #     poptcurve, pcovcurve = opt.curve_fit(gpscssmodels.curve, freqplot, fluxplot, p0 = p0curve, sigma = flux_errplot, maxfev = 10000)
        #     redchisq_curve[i] = redchisq(fluxplot,gpscssmodels.curve(freqplot,*poptcurve),flux_errplot,4)[1]
        #     # print redchisq(fluxplot,gpscssmodels.curve(freqplot,*poptcurve),flux_errplot,4)
        #     if redchisq_curve < 5.:
        #         print 'Storing as a GPS source.'
        #         gpsind.append(i)
        # except (ValueError, RuntimeError):
        #     print 'Curve fit could not find good inhomo fit.'

        # if redchisq_curve[i] < 10. and redchisq_curve[i] > 0.:
        #     userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
        #     if userinput == '' :
        #         sed([gpscssmodels.powlaw,gpscssmodels.curve],[poptpowlaw,poptcurve],freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2))+r' Inhomobremss $ \chi_{\rm{red}}$  = '+str(round(redchisq_curve[i],2)))
        # elif redchisq_pow[i] > 0.:
        #     userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
        #     if userinput == '' :
        #         sed(gpscssmodels.powlaw,poptpowlaw,freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2)))
     ...
     #     if redchisq_pow[i] > 3.:
        ...
        # try:
        #     poptsinginhomobremss, pcovsinginhomobremss = opt.curve_fit(gpscssmodels.singinhomobremss, freqplot, fluxplot, p0 = p0singinhomobremss, sigma = flux_errplot, maxfev = 10000)
        #     redchisq_curve[i] = redchisq(fluxplot,gpscssmodels.singinhomobremss(freqplot,*poptsinginhomobremss),flux_errplot,2)[1]
        #     userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
        #     if userinput == '' and redchisq_curve[i] < 50.:
        #         sed([gpscssmodels.powlaw,gpscssmodels.singinhomobremss],[poptpowlaw,poptsinginhomobremss],freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2))+r' Inhomobremss $ \chi_{\rm{red}}$  = '+str(round(redchisq_curve[i],2)))
        #     else: 
        #         sed(gpscssmodels.powlaw,poptpowlaw,freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2)))
        ...
        # except (ValueError, RuntimeError): # If curve fit can not find a good fit, skip the source.
        #     print 'Curve model can not find a good fit.'
        #     userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
        #     if userinput == '' :
        #         sed(gpscssmodels.powlaw,poptpowlaw,freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2)))
        #     continue

    # Code below does curve model, not singinhomo model.
    # if len(fluxplot) > 3. and redchisq_pow[i] > 3.:
    #     try:
    #         poptcurve, pcovcurve = opt.curve_fit(gpscssmodels.curve, freqplot, fluxplot, p0 = p0curve, sigma = flux_errplot, maxfev = 10000)
    #         redchisq_curve[i] = redchisq(fluxplot,gpscssmodels.curve(freqplot,*poptcurve),flux_errplot,2)[1]
    #         userinput = raw_input("Hit enter if wanting plot of source, ortherwise hit s to skip:") #plotting sources as we go.
    #         if userinput == '' and redchisq_curve[i] < 50.:
    #             sed([gpscssmodels.powlaw,gpscssmodels.curve],[poptpowlaw,poptcurve],freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2))+r' curve $ \chi_{\rm{red}}$  = '+str(round(redchisq_curve[i],2)))
    #         else: 
    #             sed(gpscssmodels.powlaw,poptpowlaw,freqplot,fluxplot,flux_errplot, freq_labels = True, savefig = False, title = MWACS_name[i] +r' $\alpha$ = '+str(round(alpha[i],3))+r' Powlaw $ \chi_{\rm{red}}$  = '+str(round(redchisq_pow[i],2)))
    #     except (ValueError, RuntimeError): # If curve fit can not find a good fit, skip the source.
    #         print 'Curve model can not find a good fit.'
