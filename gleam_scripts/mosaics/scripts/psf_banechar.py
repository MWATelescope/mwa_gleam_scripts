...
parser.add_option('--usemrc',action="store_true",dest="usemrc",default=False,
                  help="Use MRC (and VLSSr North of Dec 20) to select unresolved sources? (Will turn off initial gridding.)")
...
def average_region(data,xmin,xmax,ymin,ymax,params):
    ...
    weights=data['peak_flux'][newmask]/data['local_rms'][newmask]
    vals=[]
    for param in params:
        docount=False
