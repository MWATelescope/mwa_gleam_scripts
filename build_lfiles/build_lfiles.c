...
void usage() {
    ...
    fprintf(stdout,"\t-s\t <start> -- seconds from beginning of file to start\n");
...
int main(int argc, char **argv) {
    ...
    while ((arg = getopt(argc, argv, "m:i:f:F:s:v:n:o:pT:")) != -1) {
	...
	char *val, value[1000], nullstr[]="*", *ttype[1536];
	char keyword[FLEN_KEYWORD], colname[FLEN_VALUE];
	int status = 0;   /*  CFITSIO status value MUST be initialized to zero!  */
	int hdunum, hdutype, numhdus, ncols, ii, anynul, dispwidth[1000],nfound,typecode;
	int firstcol, lastcol = 0, linewidth;
	long jj, nrows,repeat,width_in_bytes;
	...
	sprintf(tmp_lccfilename,".working.LCCSPC",output_file);
	sprintf(tmp_lacfilename,".working.LACSPC",output_file);
	...
	while (ihdu < stophdu) {
		...
		if (!fits_open_file(&fptr, input_file[ifile], READONLY, &status))
		{
			...
			if ( fits_get_hdu_num(fptr, &hdunum) == 1 ) {
			    ...
			    fits_movabs_hdu(fptr, ihdu+primary, &hdutype, &status);
			    if (status != 0) {
			...
			fprintf(stdout,"table have %ld rows and %d cols\n",nrows,ncols);
			fprintf(stdout,"each col is type %d with %ld entries and %ld bytes long\n",typecode,repeat,width_in_bytes);
