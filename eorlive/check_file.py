...
def check_ms(fconnector,obsid,boolmode):
    msfile = fconnector.send_fusion_query('POST','SELECT MS_File FROM %s WHERE ObsID=%s'%(obstable_id,obsid),{'Content-Length':0})
    msfile = json.loads(rowid.read())
...
def check_uvfits(fconnector,obsid,boolmode):
    uvfile = fconnector.send_fusion_query('POST','SELECT UVFITS_File FROM %s WHERE ObsID=%s'%(obstable_id,obsid),{'Content-Length':0})
    uvfile = json.loads(rowid.read())
    uvfile = msfile['rows'][0][0]
    if(boolmode):
        if(uvfile):
            return 1
        else:
...
if __name__=='__main__':
    ...
    parser.add_argument('-u','--uvfits',metavar='uvfits path',dest='file_func',action='store_const',const=check_uvits,help='get the file path of uvfits file')
    ...
    print(args.file_func(args.obsid,args.bool_mode))
